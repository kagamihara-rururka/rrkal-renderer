...
def _to_html(
    ...
    html_lines: List[str] = [
        ...
        "      <p class='hotkeys small'>Shortcuts: J/K or [] or ←/→ (page prev/next), ↑/↓ (row nav), Shift+Enter (copy first row on current page), Home/End (jump first/last row), C/Enter (copy selected), F/T (focus symbol filters), R (reset active), 0 (clear filters)</p>",
        "    </section>",
        "    <section class='panel'>",
        ...
        "        if (evt.metaKey || evt.ctrlKey || evt.altKey || isTyping) { return; }",
        "        if (evt.shiftKey && k === 'enter') {",
        "          evt.preventDefault();",
        "          if (activeInspector === 'event') {",
        "            const list = eventFiltered;",
        "            const size = parsePageSize(eventPageSize);",
        "            const page = clamp(eventPageState.page, 1, Math.max(1, Math.ceil(list.length / size))) - 1;",
        "            const offset = page * size;",
        "            const row = list[offset] || null;",
        "            copyRowText(row ? JSON.stringify(row, null, 2) : '');",
        "          } else if (activeInspector === 'trade') {",
        "            const list = tradeFiltered;",
        "            const size = parsePageSize(tradePageSize);",
        "            const page = clamp(tradePageState.page, 1, Math.max(1, Math.ceil(list.length / size))) - 1;",
        "            const offset = page * size;",
        "            const row = list[offset] || null;",
        "            copyRowText(row ? JSON.stringify(row, null, 2) : '');",
        "          }",
