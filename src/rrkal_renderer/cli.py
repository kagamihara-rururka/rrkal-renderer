...
def _to_html(
    ...
    html_lines: List[str] = [
        ...
        "      <p class='hotkeys small'>Shortcuts: J/K or [] or ←/→ (page prev/next), ↑/↓ (row nav), Home/End (jump first/last row), C/Enter (copy selected), F/T (focus symbol filters), R (reset active), 0 (clear filters)</p>",
        "    </section>",
        "    <section class='panel'>",
        ...
        "      if (!eqChart || !eqCursor || !eqCrosshair || !chartMeta) return;",
        ...
        "      if (persist) {",
        "        localStorage.setItem(`rrkal-panel-${targetId}`, collapsed ? '1' : '0');",
        "      }",
        "    }",
