...
def _to_html(
    ...
    html_lines: List[str] = [
        ...
        "    .match{background:rgba(250,204,21,.2);padding:0 .16rem;border-radius:2px}",
        "    .inspector-table{width:100%}",
        "    .inspector-table.card-mode thead{display:none}",
        "    .inspector-table.card-mode tbody tr{display:block;padding:.25rem 0;border-bottom:1px solid #1e293b}",
        "    .inspector-table.card-mode tbody tr + tr{margin-top:.2rem}",
        "    .inspector-table.card-mode td{display:none;border:0}",
        "    .inspector-table.card-mode td.card-summary{display:block;background:#020617b8;border:1px solid #334155;border-radius:11px;padding:.5rem .6rem}",
        "    .inspector-table .card-summary{display:none}",
        "    .photo .inspector-table .card-summary{display:block}",
        "    .photo .inspector-table .card-title{display:block;font-weight:700;line-height:1.2;color:var(--text)}",
        "    .photo .inspector-table .card-sub{display:block;color:var(--text-dim);margin-top:.12rem;font-size:.75rem}",
        "    .photo .inspector-table .card-sub + .card-sub{margin-top:.06rem}",
        "    .photo .inspector-table .card-meta{display:block;margin-top:.16rem;color:#cbd5e1;font-size:.74rem;line-height:1.35}",
        "    .photo .inspector-table .card-badge{display:inline-block;padding:.12rem .42rem;border-radius:999px;border:1px solid #334155;font-size:.68rem;line-height:1;color:#d1d5db;background:rgba(15,23,42,.4);margin-right:.32rem;margin-top:.06rem}",
        "    .photo .inspector-table .card-badge.up{border-color:#22c55e66;color:#86efac}",
        "    .photo .inspector-table .card-badge.down{border-color:#ef444466;color:#fda4af}",
        "    .up{color:var(--ok)}",
        "    .down{color:var(--bad)}",
    ...
    html_lines.extend([
        ...
        "        <div class='table-wrap'><table class='inspector-table' id='eventInspectorTable'><thead><tr><th class='compact-hide'>event</th><th class='compact-hide'>symbol</th><th class='compact-hide'>timestamp</th><th>details</th></tr></thead><tbody id='eventBody'></tbody></table></div>",
        ...
        "        <div class='table-wrap'><table class='inspector-table' id='tradeInspectorTable'><thead><tr><th>symbol</th><th>direction</th><th>quantity</th><th>entry</th><th>exit</th><th>pnl</th><th>entry_cost</th><th>exit_cost</th><th>start_ts</th><th>end_ts</th></tr></thead><tbody id='tradeBody'></tbody></table></div>",
        ...
        "    const eventTable = document.querySelector('#eventInspectorTable');",
        "    const tradeBody = document.querySelector('#tradeBody');",
        "    const tradeTable = document.querySelector('#tradeInspectorTable');",
        ...
        "      }",
        "      applyInspectorViewMode();",
        "      try { localStorage.setItem('rrkal-photo-style', enabled ? '1' : '0'); } catch (e) {}",
        "    }",
        "    function applyInspectorViewMode() {",
        "      const photo = document.body.classList.contains('photo');",
        "      if (eventTable) { eventTable.classList.toggle('card-mode', photo); }",
        "      if (tradeTable) { tradeTable.classList.toggle('card-mode', photo); }",
        "    }",
        ...
        "      return `${text.slice(0, start)}<span class='match'>${text.slice(start, end)}</span>${text.slice(end)}`;",
        "    }",
        ...
        "      const searchTerm = normalizeText(globalSearch ? globalSearch.value : '');",
        "      eventBody.innerHTML = page.map((row, idx)=>{",
        "        const eventText = safeText(row.event || '');",
        "        const symbolText = safeText(row.symbol || '');",
        "        const tsText = safeText(row.timestamp || '');",
        "        const detailText = safeText(JSON.stringify(row.details || {}));",
        "        return `<tr data-row-index='${offset + idx}'>` +",
        "          `<td class='card-summary'><span class='card-title'>${highlightText(eventText, searchTerm)}</span>` +",
        "          `<span class='card-sub'><span class='card-badge'>${highlightText(symbolText, searchTerm)}</span><span>${highlightText(tsText, searchTerm)}</span></span>` +",
        "          `<span class='card-meta'>${highlightText(detailText, searchTerm)}</span></td>` +",
        "          `<td class='compact-hide'>${highlightText(eventText, searchTerm)}</td>` +",
        "          `<td class='compact-hide'><span data-symbol='${symbolText}' class='symbol-link'>${highlightText(symbolText, searchTerm)}</span></td>` +",
        "          `<td class='compact-hide'>${highlightText(tsText, searchTerm)}</td>` +",
        "          `<td class='compact-hide'><pre>${detailText}</pre></td></tr>`;",
        "      }).join('');",
        ...
        "      tradeBody.innerHTML = page.map((row, idx)=>{",
        "        const cls = Number(row.pnl || 0) >= 0 ? 'up' : 'down';",
        "        const symbolText = safeText(row.symbol || '');",
        "        const dirText = safeText(row.direction || '');",
        "        const qtyText = num(row.quantity || 0);",
        "        const entryText = num(row.entry || 0);",
        "        const exitText = num(row.exit || 0);",
        "        const pnlText = num(row.pnl || 0);",
        "        const entryCost = num(row.entry_cost || 0);",
        "        const exitCost = num(row.exit_cost || 0);",
        "        const startTs = safeText(row.start_ts || '');",
        "        const endTs = safeText(row.end_ts || '');",
        "        return `<tr data-row-index='${offset + idx}'>` +",
        "          `<td class='card-summary'><span class='card-title'><span data-symbol='${symbolText}' class='symbol-link'>${highlightText(symbolText, searchTerm)}</span> · ${highlightText(dirText, searchTerm)}</span>` +",
        "          `<span class='card-sub'><span class='card-badge ${cls}'>${highlightText(pnlText, searchTerm)}</span><span class='card-badge'>qty ${highlightText(qtyText, searchTerm)}</span></span>` +",
        "          `<span class='card-sub'>${highlightText(entryText, searchTerm)} → ${highlightText(exitText, searchTerm)}</span>` +",
        "          `<span class='card-meta'>${highlightText(startTs, searchTerm)} ~ ${highlightText(endTs, searchTerm)}</span></td>` +",
        "          `<td class='compact-hide'><span data-symbol='${symbolText}' class='symbol-link'>${highlightText(symbolText, searchTerm)}</span></td>` +",
        "          `<td class='compact-hide'>${highlightText(dirText, searchTerm)}</td>` +",
        "          `<td class='compact-hide'><span class='${cls}'>${highlightText(qtyText, searchTerm)}</span></td>` +",
        "          `<td class='compact-hide'>${highlightText(entryText, searchTerm)}</td>` +",
        "          `<td class='compact-hide'>${highlightText(exitText, searchTerm)}</td>` +",
        "          `<td class='compact-hide ${cls}'>${highlightText(pnlText, searchTerm)}</td>` +",
        "          `<td class='compact-hide'>${highlightText(entryCost, searchTerm)}</td>` +",
        "          `<td class='compact-hide'>${highlightText(exitCost, searchTerm)}</td>` +",
        "          `<td class='compact-hide'>${highlightText(startTs, searchTerm)}</td>` +",
        "          `<td class='compact-hide'>${highlightText(endTs, searchTerm)}</td></tr>`;",
        "      }).join('');",
